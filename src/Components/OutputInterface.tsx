		...
		<div className="output-section ">
			...
			<ul>
				{outputState.map((element, index) => {
					return (
						menuOpenAt == index ?
							<li key={index} className="body-table delete-menu" onClick={() => setMenuOpenAt(-1)}><span>¿Deseas eleminar este cargo?</span><button onClick={() => handleChange(undefined, index, 'delete')} className="mobile-delete-button">Si</button></li>
							:
							<li key={index} className="grid-table body-table" onClick={() => window.innerWidth < 800 ? setMenuOpenAt(index) : null}>
